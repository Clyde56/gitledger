...
export default function Login() {
    ...
    // 处理 Worker 回调（使用新参数名 gitauth，避免触发原 afterLogin 错误）
    useEffect(() => {
        const params = new URLSearchParams(window.location.search);
        const gitauthStr = params.get("gitauth");
        if (gitauthStr) {
            try {
                const tokenData = JSON.parse(decodeURIComponent(gitauthStr));
                if (tokenData.access_token) {
                    const expiresIn = Date.now() + (tokenData.expires_in || 2592000) * 1000;
                    ...
                    // reload 让 store 读取 token
                    window.location.reload();
                }
            } catch (e) {
                console.error("解析 gitauth 参数失败", e);
            }
        }
    }, []);
                    ...
                    <div className="min-h-20 h-fit pb-4 flex flex-col gap-4">
                        {loading ? (
                            ...
                            <>
                                ...
                                {/* 其余按钮（Gitee / WebDAV / S3 / Offline）保持完全不变 */}
                                <div className="flex flex-col gap-1">
                                    <button
                                        type="button"
                                    ...
                                    >
                                        <svg fill="currentColor" width="32" height="32" viewBox="0 0 24 24" role="img" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M11.984 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.016 0zm6.09 5.333c.328 0 .593.266.592.593v1.482a.594.594 0 0 1-.593.592H9.777c-.982 0-1.778.796-1.778 1.778v5.63c0 .327.266.592.593.592h5.63c.982 0 1.778-.796 1.778-1.778v-.296a.593.593 0 0 0-.592-.593h-4.15a.592.592 0 0 1-.592-.592v-1.482a.593.593 0 0 1 .593-.592h6.815c.327 0 .593.265.593.592v3.408a4 4 0 0 1-4 4H5.926a.593.593 0 0 1-.593-.593V9.778a4.444 4.444 0 0 1 4.445-4.444h8.296z" />
                                        </svg>
                                        <div className="flex-1">{t("login-to-gitee")}</div>
                                    </button>
                                    <button
                                        type="button"
                                        className="underline text-xs cursor-pointer"
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        <div className="flex-1">{t("offline-mode")}</div>
                                    </button>
                                </div>
                            </>
                        )}
...
function Guide({ className }: { className?: string }) {
    const t = useIntl();
    return (
        <div className="w-full p-4 flex-1 border-b bg-stone-800 text-white flex flex-col items-center justify-center gap-4 relative">
            ...
            <a className="absolute bottom-4 right-4 text-xs opacity-60 underline" target="_blank" href="https://glink25.github.io/post/Cent---%E4%BD%A0%E5%8F%AF%E8%83%BD%E5%8F%AA%E9%9C%80%E8%A6%81%E4%B8%80%E4%B8%AA%E8%AE%B0%E8%B4%A6%E8%BD%AF%E4%BB%B6/" rel="noopener">
                {t("see-introduce")}
            </a>
        </div>
    );
